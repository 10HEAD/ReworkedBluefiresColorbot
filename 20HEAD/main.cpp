...
void MoveMouseFromScreenPosition(Vector2 front, int height, int width) {
	...
	if(flickAim) {
		DD_movR(moveX + offset[0], moveY + offset[1]);
		Sleep(flickAimTime);
	}
	else {
		DD_movR(moveX * speed + offset[0], moveY * speed + offset[1]);
	}
}
...
int main(int, char**)
{
	...
	HMODULE hDll = LoadLibraryW(L"MouseInput.dll");    //App x64
	...
	HWND hwnd = ::CreateWindow(wc.lpszClassName, _T(" "), WS_OVERLAPPEDWINDOW, 0, 0, 400, 500, NULL, NULL, wc.hInstance, NULL);
	...
	while(msg.message != WM_QUIT)
	{
		...
		if (GetAsyncKeyState(VK_F6) & 1) trigger = !trigger;
		...
		if (enabled)
		{

			{
				...
				if (testFull360) {
					...
					ImGui::SliderInt("FovX", &maxX, 10, width / 2);
					ImGui::SliderInt("FovY", &maxY, 10, height / 2);
					ImGui::InputInt2("Offset XY", offset);
					ImGui::InputInt("Full360", &full360);
					ImGui::Text("Youknow");
					ImGui::Checkbox("Snip", &trigger);
					ImGui::Text("Recol");
					ImGui::Checkbox("Recoil", &recoil);
					ImGui::SliderFloat("##recoill", &recoilms, 0.1f, 10.f, "% .1f");
